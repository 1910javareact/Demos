...
export async function daoGetGardenById(id: number): Promise<Garden> {
    ...
    try{
        client = await connectionPool.connect()
        let result = await client.query('SELECT * FROM garden_book.garden natural join garden_book.garden_roles natural join garden_book.roles where garden_id = $1', [id])
